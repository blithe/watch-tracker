    ...
    <div>
      ...
      {logs.length > 0 ? (
        <div className="space-y-4">
          {logs.map(log => (
            <div key={log.id} className="bg-zinc-900 border border-zinc-800 rounded-xl p-6">
              ...
              <div className="mt-4 flex gap-3">
                <Link href={`/log?date=${date}&edit=${log.id}`} className="text-sm text-blue-400 hover:text-blue-300">Edit</Link>
                <DeleteLogButton id={log.id} />
              </div>
            </div>
          ))}
          <Link href={`/log?date=${date}`} className="text-blue-400 hover:text-blue-300 text-sm inline-block">+ Log another watch</Link>
        </div>
      ) : (
