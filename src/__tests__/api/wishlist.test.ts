...
describe('/api/wishlist', () => {
  ...
  describe('GET /api/wishlist', () => {
    ...
    it('should return wishlist items with latest prices', async () => {
      ...
      const result = db.prepare(`
        INSERT INTO wishlist (brand, model, reference, target_price, notes, status) 
        VALUES (?, ?, ?, ?, ?, ?)
      `).run('Rolex', 'Submariner', '116610LN', 8500, 'Dream watch', 'watching');
    ...
    it('should return items ordered by created_at DESC', async () => {
      ...
      db.prepare(`
        INSERT INTO wishlist (brand, model, created_at) 
        VALUES (?, ?, ?)
      `).run('Omega', 'Speedmaster', '2026-02-08 10:00:00');
      
      db.prepare(`
        INSERT INTO wishlist (brand, model, created_at) 
        VALUES (?, ?, ?)
      `).run('Rolex', 'Submariner', '2026-02-08 12:00:00');
  ...
  describe('PATCH /api/wishlist', () => {
    it('should update an existing wishlist item', async () => {
      const db = getTestDb();
      const result = db.prepare(`
        INSERT INTO wishlist (brand, model, reference, target_price, status) 
        VALUES (?, ?, ?, ?, ?)
      `).run('Rolex', 'Submariner', '116610LN', 8500, 'watching');
  ...
  describe('DELETE /api/wishlist', () => {
    it('should delete a wishlist item and its price history', async () => {
      ...
      const response = await DELETE(new NextRequest(`http://localhost/api/wishlist?id=${wishlistId}`));
      const data = await response.json();
    ...
    it('should return 400 when id is missing', async () => {
      const response = await DELETE(new NextRequest('http://localhost/api/wishlist'));
      const data = await response.json();
    ...
    it('should return 404 when item does not exist', async () => {
      const response = await DELETE(new NextRequest('http://localhost/api/wishlist?id=999'));
      const data = await response.json();
