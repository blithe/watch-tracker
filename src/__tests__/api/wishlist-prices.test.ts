...
describe('/api/wishlist/[id]/prices', () => {
  ...
  describe('GET /api/wishlist/[id]/prices', () => {
    it('should return empty array when no price history exists', async () => {
      ...
      `).run(TEST_USER_ID, 'Rolex', 'Submariner');

      const response = await GET(
        makeReq('http://localhost'),
        { params: { id: String(result.lastInsertRowid) } } as any
      );
    ...
    it('should return price history sorted by recorded_at DESC', async () => {
      ...
      const response = await GET(
        makeReq('http://localhost'),
        { params: { id: String(wishlistId) } } as any
      );
      const data = await response.json();
  ...
  describe('POST /api/wishlist/[id]/prices', () => {
    it('should create a new price entry with all fields', async () => {
      ...
      const request = makeReq('http://localhost/api/wishlist/1/prices', {
        method: 'POST',
        body: JSON.stringify(requestBody)
      });

      const response = await POST(request, { params: { id: wishlistId } } as any);
      const data = await response.json();
    ...
    it('should return 404 when wishlist item does not exist', async () => {
      const request = makeReq('http://localhost/api/wishlist/999/prices', {
        method: 'POST',
        body: JSON.stringify({ price: 1000 })
      });

      const response = await POST(request, { params: { id: '999' } } as any);
      const data = await response.json();
    ...
    it('should return 400 when wishlist id is missing', async () => {
      const request = makeReq('http://localhost/api/wishlist//prices', {
        method: 'POST',
        body: JSON.stringify({ price: 1000 })
      });

      const response = await POST(request, { params: { id: '' } } as any);
      const data = await response.json();
